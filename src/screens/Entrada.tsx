  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center gap-6 px-6">
      <div className="text-center">
        ...
        <h1 className="mt-4 text-2xl font-extrabold tracking-tight text-[var(--t1)]">Bolão da Copa</h1>
        <p className="mt-1 text-sm text-[var(--t2)]">Palpite nos jogos e dispute com a galera.</p>
      </div>
      ...
      {mode === 'novo' && (
        <div className="flex flex-col gap-3">
          <label className="text-sm text-[var(--t2)]">Como você quer aparecer no ranking?</label>
          <input
            autoFocus
            value={name}
          ...
          <button onClick={backToMenu} className="text-sm text-[var(--t3)] transition-colors active:text-[var(--t2)]">
            ← voltar
          </button>
        </div>
      )}
      ...
      {mode === 'existente' && !selected && (
        <div className="flex flex-col gap-3">
          ...
          {participants.length === 0 ? (
            <p className="text-sm text-[var(--t3)]">Ninguém se cadastrou ainda. Use "Sou novo aqui".</p>
          ) : (
