    ...
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between px-1 pt-2">
        ...
        <div className="text-right">
          <span className="text-xl font-extrabold text-canarinho-yellow">{total}</span>
          <span className="ml-1 text-xs text-emerald-300/60">pts</span>
        </div>
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <ul className="flex flex-col gap-2">
          {rows.map(({ pred, match, pts }) => {
            ...
            return (
              <li key={pred.id} className="rounded-xl border border-emerald-900/60 bg-emerald-950/40 px-3 py-2.5">
                <div className="flex items-center justify-between text-[11px] text-emerald-300/50">
                  <span>{match.label ?? ''}</span>
                  <span>{formatKickoff(match.kickoff)}</span>
                </div>
                <div className="mt-1 flex items-center justify-between">
                  <span className="truncate text-sm text-emerald-50">
                    {match.home_team ?? 'A definir'} <span className="text-emerald-600">×</span>{' '}
                    {match.away_team ?? 'A definir'}
                  </span>
                </div>
                <div className="mt-1 flex items-center justify-between text-sm">
                  <span className="text-emerald-200/80">
                    Palpite: <strong>{pred.home_score} × {pred.away_score}</strong>
                    {finished && (
                  ...
                  {finished ? (
                    ...
                    <span className="text-xs text-emerald-300/50">{locked ? 'aguardando resultado' : 'aberto'}</span>
                  )}
                </div>
              </li>
            )
...
function Badge({ pts }: { pts: number | null }) {
  ...
  const color =
    pts === 10 ? 'bg-canarinho-yellow text-emerald-950' : pts === 5 ? 'bg-canarinho-green text-white' : 'bg-emerald-900 text-emerald-300/70'
