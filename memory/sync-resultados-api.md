...
- **Onde roda:** Edge Function `supabase/functions/sync-resultados` (Deno), agendada por `pg_cron` a cada 30 min; `supabase/functions/sync-ao-vivo` cobre o placar ao vivo (inclusive prorrogação/pênaltis) a cada minuto. Agendamento formalizado em `supabase/migrations/0009_sync_observability_cron.sql` (segredo `CRON_SECRET` no Vault, não mais anon key hardcoded — o cron antigo apontava pra uma URL de projeto errada e nunca funcionou). Token da API fica como secret `FOOTBALL_DATA_TOKEN`, nunca no frontend. Diagnóstico via tabela `sync_logs`.
